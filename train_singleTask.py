...
def main(save_path):
    train_dataset = DATASET(mod='train', task=task)
    val_dataset = DATASET(mod='val', task=task)
...
def test(save_path, mod='test'):
    test_dataset = DATASET(mod=mod, task=task)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--dataset', type=str, default='UDIVA')
    parser.add_argument('-t', '--task', type=str, default='A', choices=['T', 'A', 'L', 'G'])
    ...
    if dataset_name == 'UDIVA':
        batch_size = 8
        text_len = 452
        from utils_singleTask import myDataset_UDIVA_singleTask as DATASET
    else:
        exit()
    ...
    save_path = './result/{}_singleTask_8-{}_{}/{}'
    ...
    for i in range(times):
        path = save_path.format(dataset_name, task, ind, i)
        print(path)
        # 训练和验证
        # main(path)
        # test
    ...
    for r in res:
        ...
        print(
            f'ACC: {r[4]:.4f} | {r[5]:.4f} | CCC: {r[6]:.4f} | R2: {r[7]:.4f} | mean: {meant:.4f}')
